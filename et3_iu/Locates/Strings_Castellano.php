<?php
//CADENAS UTILIZADAS EN CASTELLANO
$strings = array(
'Inserción realizada con éxito' => 'Inserción realizada con éxito',
'El usuario ya existe en la base de datos' => 'El usuario ya existe en la base de datos',
	'El usuario ya existe en la base de datos' => 'El usuario ya existe en la base de datos',
'No se ha podido conectar con la base de datos' => 'No se ha podido conectar con la base de datos',
'Introduzca un valor para el dni del usuario' => 'Introduzca un valor para el dni del usuario',
	'Introduzca un valor para el dni del usuario' => 'Introduzca un valor para el dni del usuario',
'Volver' => 'Volver',
'Error en la consulta sobre la base de datos' => 'Error en la consulta sobre la base de datos',
'El usuario ha sido borrado correctamente' => 'El usuario ha sido borrado correctamente',
	'El usuario ha sido borrado correctamente' => 'El usuario ha sido borrado correctamente',
'El usuario no existe' => 'El usuario no existe',
	'El usuario no existe' => 'El usuario no existe',
'El usuario se ha modificado con éxito' => 'El usuario se ha modificado con éxito',
	'El usuario se ha modificado con éxito' => 'El usuario se ha modificado con éxito',
	'Se ha producido un error en la modificación del usuario' => 'Se ha producido un error en la modificación del usuario',
	'Se ha producido un error en la modificación del usuario' => 'Se ha producido un error en la modificación del usuario',
	'La contraseña para este usuario es erronea'=>'La contraseña para este usuario es erronea',
	'El usuario ya no se encuentra contratado'=>'El usuario ya no se encuentra contratado',
	'Insertar usuario'=>'Insertar usuario',
	'Modificar usuario'=>'Modificar usuario',
	"La contraseña para este usuario es errónea"=>"La contraseña para este usuario es errónea",
	'USUARIO_USER'=>'Usuario',
	'USUARIO_PASSWORD'=>'Contraseña',
	'USUARIO_FECH_NAC'=>'Fecha de Nacimiento',
	'USUARIO_EMAIL'=>'Email',
	'USUARIO_NOMBRE'=>'Nombre',
	'USUARIO_APELLIDO'=>'Apellidos',
	'USUARIO_DNI'=>'DNI',
	'USUARIO_TELEFONO'=>'Teléfono',
	'USUARIO_CUENTA'=>'Cuenta',
	'USUARIO_DIRECCION'=>'Dirección',
	'USUARIO_COMENTARIOS'=>'Comentarios',
	'USUARIO_LESIONES'=>'Lesiones',
	'USUARIO_TIPO'=>'Tipo',
	'USUARIO_ESTADO'=>'Estado',
	'USUARIO_FOTO'=>'Foto',
	'USUARIO_USER2'=>'Búsqueda por Usuario:',
	'USUARIO_DNI2'=>'Búsqueda por DNI:',
	'USUARIO_NOMBRE2'=>'Búsqueda por Nombre',
	'USUARIO_APELLIDO2'=>'y Apellido:',
'Insertar' => 'Insertar',
'Modificar' => 'Modificar',
'Borrar' => 'Borrar',
'Consultar' => 'Consultar',
    'Registro'=>'Registro',
'Update' => 'Update',
'Delete' => 'Delete',
'Cerrar Sesión' => 'Cerrar Sesión',
	'La pagina ya existe en la base de datos' => 'La página ya existe en la base de datos',
	'Introduzca un valor para la clave de la pagina' => 'Introduzca un valor para la clave de la página',
	'La pagina ha sido borrada correctamente' => 'La página ha sido borrada correctamente',
	'La pagina no existe' => 'La página no existe',
	'La pagina se ha modificado con exito' => 'La página se ha modificado con éxito',
	'Se ha producido un error en la modificacion de la pagina' => 'Se ha producido un error en la modificación de la página',
	'PAGINA_ID' => 'Clave de la página',
	'PAGINA_LINK' => 'Link de la página',
	'PAGINA_NOM' => 'Nombre de la página',
	'Borrar' => 'Borrar',
	'Consultar' => 'Consultar',
	'Listar' => 'Listar',
	'Páginas' => 'Páginas',
	'Paginas' => 'Páginas',
	'(Los campos "Link de la pagina" y "Nombre de la pagina" NO son necesarios)' => '(Los campos "Link de la página" y "Nombre de la página" NO son necesarios)',
	'La funcionalidad ya existe en la base de datos' => 'La funcionalidad ya existe en la base de datos',
	'Introduzca un valor para la clave de la funcionalidad' => 'Introduzca un valor para la clave de la funcionalidad',
	'La funcionalidad ha sido borrada correctamente' => 'La funcionalidad ha sido borrada correctamente',
	'La funcionalidad no existe' => 'La funcionalidad no existe',
	'La funcionalidad se ha modificado con exito' => 'La funcionalidad se ha modificado con áxito',
	'Se ha producido un error en la modificacion de la funcionalidad' => 'Se ha producido un error en la modificación de la funcionalidad',
	'Funcionalidad' => 'Funcionalidad',
	'Funcionalidades' => 'Funcionalidades',
	'(Los campos "Link de la funcionalidad" y "Nombre de la funcionalidad" NO son necesarios)' => '(Los campos "Link de la funcionalidad" y "Nombre de la funcionalidad" NO son necesarios)',
	'Fallo al conectar a MySQL: (' => 'Fallo al conectar a MySQL: (',
	') ' => ') ',
	'Anadida con exito' => 'Añadida con exito',
	'Insertar Rol'=>'Insertar Rol',
	'ROL_ID' => 'ID del Rol',
	'ROL_NOM' => 'Nombre del ROL',
	'GESTION usuarios'=>'Gestión de Usuarios',
	'GESTION ROLES'=>'Gestión de roles',
	'GESTION FUNCION'=>'Gestión de funcionalidades',
	'Modificar Rol'=>'Modificar Rol',
	'No funcionalidad'=>'Seleccione alguna funcionalidad',
	'El rol se ha modificado con éxito'=>'El rol se ha modificado con éxito',
	'Borrar Rol'=>'Borrar Rol',
	'El rol ha sido borrado correctamente'=>'El rol ha sido borrado correctamente',
	'El rol no existe'=>'El rol no existe',
	'Páginas' => 'Páginas',
	'Insertar Funcionalidad'=>'Insertar Funcionalidad',
	'FUNCIONALIDAD_ID' => 'ID del la funcionalidad',
	'FUNCIONALIDAD_NOM' => 'Nombre de la funcionalidad',
	'FUNCION_ID' => 'ID del la funcionalidad',
	'FUNCION_NOM' => 'Nombre de la funcionalidad',
	'GESTION usuarioS'=>'Gestión de Usuarios',
	'GESTION ROLES'=>'Gestión de roles',
	'GESTION FUNCION'=>'Gestión de funcionalidades',
	'Modificar Funcionalidad'=>'Modificar Funcionalidad',
	'No pagina'=>'Seleccione alguna página',
	'La funcionalidad se ha modificado con éxito'=>'La funcionalidad se ha modificado con éxito',
	'Borrar Funcionalidad'=>'Borrar Funcionalidad',
	'La funcionalidad ha sido borrada correctamente'=>'La funcionalidad ha sido borrada correctamente',
	'La funcionalidad no existe'=>'La funcionalidad no existe',
	'Insertar Funcionalidad'=>'Insertar Funcionalidad',
	'Modificar Funcionalidad'=>'Modificar Funcionalidad',
	'Consultar Funcionalidad'=>'Consultar Funcionalidad',
	'Consultar usuario'=>'Consultar usuario',
	'Consultar Rol'=>'Consultar Rol',
	'Consultar Pagina'=>'Consultar Página',
	'Listar' => 'Listar',
	'Ver' => 'Ver',
	'Pagina' => 'Pagina',
	'Página' => 'Página',
	'Insertar Pagina'=>'Insertar Página',
	'Anadida con exito'=>'Página añadida con éxito',
	'Modificar Pagina'=>'Modificar Página',
	'La pagina no existe'=>'La página no existe',
	'La pagina se ha modificado con exito'=>'La página se ha modificado con éxito',
	'Borrar Pagina'=>'Borrar Pagina',
	'Borrar usuario'=>'Borrar usuario',
	'Borrar Rol'=>'Borrar Rol',
	'Borrar Funcionalidad'=>'Borrar Funcionalidad',
	'Modificar Acciones'=>'Modificar Acciones',
	'No puedes borrar tu propio rol'=>'No puedes borrar tu propio rol',
	'Consultar pagina'=>'Consultar Página',
	'La pagina ha sido borrada correctamente' => 'La pagina ha sido borrada correctamente',
	'La página no existe' => 'La página no existe',
	'La pagina se ha modificado con éxito' => 'La pagina se ha modificado con éxito',
	'Se ha producido un error en la modificación de la pagina' => 'Se ha producido un error en la modificación de la pagina',
	'Introduzca un valor para la clave de la pagina' => 'Introduzca un valor para la clave de la pagina',
	'La pagina ya existe en la base de datos' => 'La pagina ya existe en la base de datos',
	'Gestión de usuarios'=>'Gestión de Usuarios',
	'Gestión de Funcionalidades'=>'Gestión de Funcionalidades',
	'Gestión de Roles'=>'Gestión de Roles',
	'Gestión de Páginas'=>'Gestión de Páginas',
	'PROFESOR'=>'PROFESOR',
	'ALUMNO'=>'ALUMNO',
	'PROFESOR RESPONSABLE'=>'PROFESOR RESPONSABLE',
	'GESTION PAGINAS'=>'GESTIÓN PÁGINAS',
	'GESTION ROLES'=>'GESTIÓN ROLES',
	'GESTION FUNCIONALIDADES'=>'GESTIÓN FUNCIONALIDADES',
	'USUARIO_TIPO2'=>'Tipo',
	'No tienes los permisos necesarios'=>'No tienes los permisos necesarios',
	'Consulta de usuarios'=>'Consulta de usuarios',
	'Modificar acciones'=>'Acciones',
	'Dni erroneo, la letra del NIF no se corresponde'=>'Dni erroneo, la letra del NIF no se corresponde',
	'Dni erroneo, formato no válido'=>'Dni erroneo, formato no válido',
	'Introduzca un valor para el nombre'=>'Introduzca un valor para el nombre',
	'Introduzca un valor para la contraseña'=>'Introduzca un valor para la contraseña',
	'Introduzca un valor  para la fecha de nacimiento'=>'Introduzca un valor  para la fecha de nacimiento',
	'¿Viene del futuro?'=>'¿Viene del futuro?',
	'Introduzca una dirección de email válida'=>'Introduzca una dirección de email válida',
	'Tiene que escribir un teléfono de 9 dígitos'=>'Tiene que escribir un teléfono de 9 dígitos',
	'Introduzca un valor correcto para el numero de CCC'=>'Introduzca un valor correcto para el numero de CCC',
	'Introduzca dirección'=>'Introduzca dirección',
	'ADMINISTRADOR'=>'ADMIN',
	'MONITOR'=>'MONITOR',
	'SECRETARIO'=>'SECRETARIO',
	'Menu Principal'=>'Menú Principal',
	'Centro deportivo'=>'Centro deportivo',
	'Calendario'=>'Calendario',
	'Gestiones Principales'=>'Gestiones Principales',
	'Asistencia Monitores'=>'Asistencia Monitores',
	'Gestiones Secundarias'=>'Gestiones Secundarias',
	'Usuario'=>'Usuario',
	'HOME'=>'INICIO',
	'ABOUT'=>'SOBRE NOSOTROS',
	'CONTACT'=>'CONTACTO',
	'MENU'=>'MENÚ GENERAL',
	'Educacion'=>'Educación Profesional',
	'Online'=>'Online',
	'2016'=>'desde 2016',
	'Estudia'=>'Estudia',
	'Con Nosotros'=>'Con Nosotros',
    'Gestión de Rúbricas'=>'Gestión de Rúbricas',
        'Rubrica creada correctamente'=>'Rubrica creada correctamente',
    'Rubrica modificada correctamente'=>'Rubrica modificada correctamente',
    'La rúbrica ha sido borrada correctamente'=>'La rúbrica ha sido borrada correctamente',
	'texto'=>'es una iniciativa de la asignatura Interfaces de Usuario, desarrollada por el Grupo 2, que pretende ser un servicio de complemento a la docencia, basado en Internet como entorno y que se dirige, por una parte, al profesorado, alumnado y personal de administración y servicios y, por otra, al alumnado preuniversitario que tenga en mente cursar  estudios superiores.',
	'Gestión de Documentación' => 'Gestión de Documentación',
	'DOCUMENTACION_ID' => 'ID',
	'DOCUMENTACION_NOM' => 'Nombre', 
	'DOCUMENTACION_PROFESOR' => 'Profesor', 
	'DOCUMENTACION_MATERIA' => 'Materia', 
	'DOCUMENTACION_FECHA' => 'Fecha', 
	'DOCUMENTACION_ENLACE' => 'Enlace',
	'DOCUMENTACION_CATEGORIA' => 'Categoría',
	'Borrar documentación' => 'Borrar documentación',
	'Insertar documentación' => 'Insertar documentación',
	'El documento ha sido borrado correctamente'=>'El documento ha sido borrado correctamente',
	'Modificar Documentación' => 'Modificar Documentación',
	'Se ha producido un error en la modificación del documento' => 'Se ha producido un error en la modificación del documento',
	'El documento se ha modificado con éxito' => 'El documento se ha modificado con éxito',
	'El documento no existe' => 'El documento no existe',
	'Consultar Documentación' => 'Consultar Documentación',
	'Ver Documentación' => 'Ver Documentación',
	'Aún no hay documentos disponibles para esta materia' => 'Aún no hay documentos disponibles para esta materia',
//Entregas
	'GESTION ENTREGAS'=>'GESTIÓN ENTREGAS',
	'Gestión de Entregas'=>'Gestión de Entregas',
	'ENTREGA_ID'=>'ID ENTREGA',
	'ENTREGA_TRABAJO'=>'NOMBRE TRABAJO',
	'ENTREGA_HORA'=>'HORA ENTREGA',
	'ENTREGA_FECHA'=>'FECHA ENTREGA',
	'ENTREGA_ALUMNO'=>'ID ALUMNO',
	'HORAS_DEDIC'=>'HORAS DEDICADAS',
	'ENTREGA_NOMBRE'=>'NOMBRE ENTREGA',
	'DOCUMENTO' => 'ENTREGA ARCHIVO',
	'Borrar entrega'=>'Borrar entrega',
	'Insertar entrega'=>'Insertar entrega',
	'La entrega ya existe en la base de datos' => 'La entrega ya existe en la base de datos',
	'La entrega se ha borrado correctamente' => 'La entrega se ha borrado correctamente',
	'La entrega no existe' => 'La entrega no existe',
	'Introduzca un valor para nombre entrega de la entrega' => 'Introduzca un valor para nombre entrega de la entrega',
	'Modificar entrega' => 'Modificar entrega',
	'Se ha producido un error en la modificación de la entrega' => 'Se ha producido un error en la modificación de la entrega',
	'La entrega se ha modificado con éxito' => 'La entrega se ha modificado con éxito',
	'Consultar entrega' => 'Consultar entrega',
	'ENTREGA_NOMBRE2' => 'Búsqueda por nombre',
	'ENTREGA_ID2' => 'Búsqueda por ID de la entrega',
	'ENTREGA_TRABAJO2' => 'Búsqueda por ID del trabajo',
	'ENTREGA_ALUMNO2' => 'Búsqueda por ID del alumno',
	'Descargar'=>'Descargar',
	'Ver'=>'Ver',
	'Añadir'=>'Añadir',
	
    // ITEM
     'Items'=>'Items',
     'Nivel'=>'Ver Niveles',
    'ITEM_ID'=>'ID',
    'ITEM_NOMBRE'=>'Titulo',
    'ITEM_RUBRICA'=>'Rúbrica',
    'ITEM_PORCENTAJE'=>'Valor(%)',
    'Insertar Item'=>'Insertar Item',
    'Item creada correctamente'=>'Item creado correctamente',
    'Borrar Item'=>'Borrar Item',
    'Item borrado correctamente'=>'Item borrado correctamente',
    'Modificar Item'=>'Modificar Item',
    'Item modificado correctamente'=>'Item modificado correctamente',
    'Consultar Item'=>'Consultar Item',
    
  
    
    
    
    'Items' => 'Items',
    'Nivel' => 'Ver Nivel',
    'ITEM_ID' => 'ID',
    'ITEM_NOMBRE' => 'Titulo',
    'ITEM_RUBRICA' => 'Rúbrica',
    'ITEM_PORCENTAJE' => 'Valor(%)',
    'Insertar Item' => 'Insertar Item',
    'Item creada correctamente' => 'Item creado correctamente',
    'Borrar Item' => 'Borrar Item',
    'Item borrado correctamente' => 'Item borrado correctamente',
    'Modificar Item' => 'Modificar Item',
    'Item modificado correctamente' => 'Item modificado correctamente',
    'Consultar Item' => 'Consultar Item',

   

    //TRABAJOS
    'TRABAJO_ID' => 'ID',
    'TRABAJO_NOM' => 'NOMBRE',
    'TRABAJO_DESCRIPCION' => 'DESCRIPCION',
    'TRABAJO_MATERIA' => 'MATERIA',
    'TRABAJO_PROFESOR' => 'PROFESOR',
    'TRABAJO_FECHA_INICIO' => 'INICIO',
    'TRABAJO_FECHA_FIN' => 'FIN',
    'TRABAJO_FECHA_CREACION' => 'CREACION',
    'INSERTAR TRABAJO' => 'Insertar Trabajo',
    'BORRAR TRABAJO' => 'Borrar Trabajo',
    'MODIFICAR TRABAJO' => 'Editar Trabajo',
    'CONSULTAR TRABAJO' => 'Consultar Trabajo',
    'Trabajo creado correctamente' => 'Trabajo creado correctamente',
    'El trabajo ha sido borrado correctamente'=>'Trabajo borrado correctamente',
    'Trabajo modificado correctamente'=>'Trabajo modificado correctamente',
    'Gestion de trabajos'=>'Gestión de Trabajos',


    'MATERIA_ID' => 'ID',
    'MATERIA_NOM' => 'Nombre',
    'MATERIA_CREDITOS' => 'Créditos',
    'MATERIA_DEPARTAMENTO' => 'Departamento',
    'MATERIA_TITULACION' => 'Titulación',
    'MATERIA_DESCRIPCION' => 'Descripción',
    'Gestión de Materias' => 'Gestión de Materias',
	
	
    //GESTIÓN DE CORRECCIONES
    'CORRECCION_ID' => 'ID de la corrección',
	'CORRECCION_NOM' => 'Nombre de la corrección',
	'CORRECCION_ENTREGA' => 'ID de la entrega',
	'CORRECCION_RUBRICA' => 'ID de la rubrica',
	'CORRECCION_PROFESOR' => 'ID del profesor',
	'CORRECCION_NOTA' => 'Nota',
	'CORRECCION_COMENTARIOS' => 'Comentarios de la correccion',
	'RUBRICA_ID' => 'ID de la rubrica',
	'RUBRICA_AUTOR' => 'Autor de la rubrica',
	'RUBRICA_NOMBRE' => 'Nombre de la rubrica',
	
	'Borrar correccion' => 'Borrar corrección',
	'Consultar correccion' => 'Consultar corrección',
	'Modificar correccion' => 'Modificar corrección',
	'La correccion ha sido borrada correctamente' => 'La correccion ha sido borrada correctamente',
	'inserción realizada con éxito ' => 'La corrección ha sido insertada correctamente',
	'La correccion no existe' => 'La correccion no existe',
	'Consultar correcciones' => 'Consultar correcciones',
	'Gestión de correcciones' => 'Gestión de correcciones',

    'ITEM' => 'Item: ',
    'RUBRICA' => 'Rúbrica:  ',
    'NIVEL_ID' => 'ID',
    'NIVEL_ITEM' => 'Item',
    'NIVEL_DESCRIPCION' => 'Descripción',
    'NIVEL_RUBRICA' => 'Rúbrica',
    'NIVEL_ITEM' => 'Item',
    'NIVEL_PORCENTAJE' => 'Valor (%)',
    'Modificar Nivel' => 'Modificar Nivel',
    'Nivel modificado correctamente' => 'Nivel modificado correctamente',
    'Consultar Nivel' => 'Consultar Nivel',

    //TRABAJOS
    'TRABAJO_ID' => 'ID',
    'TRABAJO_NOM' => 'NOMBRE',
    'TRABAJO_DESCRIPCION' => 'DESCRIPCION',
    'TRABAJO_MATERIA' => 'MATERIA',
    'TRABAJO_PROFESOR' => 'PROFESOR',
    'TRABAJO_FECHA_INICIO' => 'INICIO',
    'TRABAJO_FECHA_FIN' => 'FIN',
    'TRABAJO_FECHA_CREACION' => 'CREACION',
    'INSERTAR TRABAJO' => 'Insertar Trabajo',
    'BORRAR TRABAJO' => 'Borrar Trabajo',
    'MODIFICAR TRABAJO' => 'Editar Trabajo',
    'CONSULTAR TRABAJO' => 'Consultar Trabajo',
    'Trabajo creado correctamente' => 'Trabajo creado correctamente',
    'El trabajo ha sido borrado correctamente'=>'Trabajo borrado correctamente',
    'Trabajo modificado correctamente'=>'Trabajo modificado correctamente',
    'Gestion de trabajos'=>'Gestión de Trabajos',


    'MATERIA_ID' => 'ID',
    'MATERIA_NOM' => 'Nombre',
    'MATERIA_CREDITOS' => 'Créditos',
    'MATERIA_DEPARTAMENTO' => 'Departamento',
    'MATERIA_TITULACION' => 'Titulación',
    'MATERIA_DESCRIPCION' => 'Descripción',
    'Gestión de Materias' => 'Gestión de Materias',
	
	
    //GESTIÓN DE CORRECCIONES
    'CORRECCION_ID' => 'ID de la corrección',
	'CORRECCION_NOM' => 'Nombre de la corrección',
	'CORRECCION_ENTREGA' => 'ID de la entrega',
	'CORRECCION_RUBRICA' => 'ID de la rubrica',
	'CORRECCION_PROFESOR' => 'ID del profesor',
	'CORRECCION_NOTA' => 'Nota',
	'CORRECCION_COMENTARIOS' => 'Comentarios de la correccion',
	'RUBRICA_ID' => 'ID de la rubrica',
	'RUBRICA_AUTOR' => 'Autor de la rubrica',
	'RUBRICA_NOMBRE' => 'Nombre de la rubrica',
	
	'Borrar correccion' => 'Borrar corrección',
	'Consultar correccion' => 'Consultar corrección',
	'Modificar correccion' => 'Modificar corrección',
	'La correccion ha sido borrada correctamente' => 'La correccion ha sido borrada correctamente',
	'inserción realizada con éxito' => 'La corrección ha sido insertada correctamente',
	'La correccion se ha modificado con éxito' => 'La corrección fue modificada con éxito',
	'La correccion no existe' => 'La correccion no existe',
	'Consultar correcciones' => 'Consultar correcciones',
	'Gestión de correcciones' => 'Gestión de correcciones',
	//Inscripciones
	'GESTION INSCRIPCIONES'=>'GESTIÓN INSCRIPCIONES',
	'Gestión de Inscripciones'=>'Gestión de Inscripciones',
	'INSCRIPCION_ID'=>'ID INSCRIPCIÓN',
	'INSCRIPCION_ALUMNO'=>'ALUMNO INSCRIPCIÓN',
	'INSCRIPCION_MATERIA'=>'MATERIA INSCRIPCIÓN',
	'Borrar inscripción'=>'Borrar inscripción',
	'Insertar inscripción'=>'Insertar inscripción',
	'Aceptar inscripción' => 'Aceptar inscripción',
	'Aceptar' => 'Aceptar',
	'La inscripción ha sido aceptada correctamente' => 'La inscripción ha sido aceptada correctamente',
	'La inscripción ya existe en la base de datos' => 'La inscripción ya existe en la base de datos',
	'La inscripción se ha borrado correctamente' => 'La inscripción se ha borrado correctamente',
	'La inscripción no existe' => 'La inscripción no existe',
	'Modificar inscripción' => 'Modificar inscripción',
	'Se ha producido un error en la modificación de la inscripción' => 'Se ha producido un error en la modificación de la inscripción',
	'La inscripción se ha modificado con éxito' => 'La inscripción se ha modificado con éxito',
	'Consultar inscripción' => 'Consultar inscripción',
	'Inscripción añadida con exito' => 'Inscripción añadida con exito',
	'La inscripción ha sido borrada correctamente' => 'La inscripción ha sido borrada correctamente',
	'Inscripción modificada correctamente' => 'Inscripción modificada correctamente',
	'No existe ninguna inscripción con los parámetros introducidos' => 'No existe ninguna inscripción con los parámetros introducidos',
	'Los parámetros introducidos no pertenecen a ninguna inscripción' => 'Los parámetros introducidos no pertenecen a ninguna inscripción',
	'No puedes borrar una inscripción ajena' => 'No puedes borrar una inscripción ajena',
	'No puedes modificar una inscripción ajena' => 'No puedes modificar una inscripción ajena',
	'Inscribirse'=>'Inscribirse',
	'Desinscribirse'=>'Desinscribirse',
	'inscrito con exito'=>'inscrito con exito',
	'ya inscrito'=>'ya inscrito',
	'desinscrito con exito'=>'desinscrito con exito',
	'no inscrito'=>'no inscrito',
	'matriculado con exito'=>'matriculado con exito',
	'ya matriculado'=>'ya matriculado',
	'desmatriculado con exito'=>'desmatriculado con exito',
	'no matriculado'=>'no matriculado',
	'Matricularse'=>'Matricular',
	'Desmatricularse'=>'Desmatricular',
	'Matriculados'=>'Matriculados',
	//Materias
	'GESTION MATERIAS'=>'GESTIÓN MATERIAS',
	'Gestión de Materias'=>'Gestión de Materias',
	'MATERIA_ID'=>'ID MATERIA',
	'MATERIA_NOM'=>'NOMBRE MATERIA',
	'MATERIA_TITULACION'=>'TITULACION MATERIA',
	'MATERIA_RESPONSABLE'=>'RESPONSABLE MATERIA',
	'MATERIA_CREDITOS'=>'CREDITOS MATERIA',
	'MATERIA_DEPARTAMENTO'=>'DEPARTAMENTO MATERIA',
	'MATERIA_RESPONSABLE'=>'RESPONSABLE MATERIA',
	'MATERIA_DESCRIPCION'=>'DESCRIPCION MATERIA',
	'Borrar materia'=>'Borrar materia',
	'Insertar materia'=>'Insertar materia',
	'La materia ya existe en la base de datos' => 'La materia ya existe en la base de datos',
	'La materia se ha borrado correctamente' => 'La materia se ha borrado correctamente',
	'La materia no existe' => 'La materia no existe',
	'Introduzca un valor para nombre de la materia' => 'Introduzca un valor para nombre de la materia',
	'Modificar materia' => 'Modificar materia',
	'Se ha producido un error en la modificación de la materia' => 'Se ha producido un error en la modificación de la materia',
	'La materia se ha modificado con éxito' => 'La materia se ha modificado con éxito',
	'Consultar materia' => 'Consultar materia',
	'MATERIA_NOM2' => 'Búsqueda por nombre',
	'MATERIA_ID2' => 'Búsqueda por ID de la materia',
	'Materia añadida con exito' => 'Materia añadida con exito',
	'La materia ha sido borrada correctamente' => 'La materia ha sido borrada correctamente',
	'Materia modificada correctamente' => 'Materia modificada correctamente',
	'Profesores'=>'Añadir Profesores',
	'confirmacion profesores'=>'Profesores añadidos correctamente',
	'Profes'=>'Profesores',
	//Matrículas
	'GESTION MATRICULAS'=>'GESTIÓN MATRÍCULAS',
	'Gestión de Matrículas'=>'Gestión de Matrículas',
	'MATRICULA_ID'=>'ID MATRÍCULA',
	'MATRICULA_ALUMNO'=>'ALUMNO MATRÍCULA',
	'MATRICULA_MATERIA'=>'MATERIA MATRÍCULA',
	'Borrar matrícula'=>'Borrar matrícula',
	'Insertar matrícula'=>'Insertar matrícula',
	'La matrícula ya existe en la base de datos' => 'La matrícula ya existe en la base de datos',
	'La matrícula se ha borrado correctamente' => 'La matrícula se ha borrado correctamente',
	'La matrícula no existe' => 'La matrícula no existe',
	'Modificar matrícula' => 'Modificar matrícula',
	'Se ha producido un error en la modificación de la matrícula' => 'Se ha producido un error en la modificación de la matrícula',
	'La matrícula se ha modificado con éxito' => 'La matrícula se ha modificado con éxito',
	'Consultar matrícula' => 'Consultar matrícula',
	'Matrícula añadida con exito' => 'Matrícula añadida con exito',
	'La matrícula ha sido borrada correctamente' => 'La matrícula ha sido borrada correctamente',
	'Matrícula modificada correctamente' => 'Matrícula modificada correctamente',
	'No existe ninguna matrícula con los parámetros introducidos' => 'No existe ninguna matrícula con los parámetros introducidos',
	'Los parámetros introducidos no pertenecen a ninguna matrícula' => 'Los parámetros introducidos no pertenecen a ninguna matrícula'
)
?>
